...
const LESSONS = [
  ...
  { id: 'ss02', title: 'SS02 selective state-space block: the token decides what the state keeps', path: 'fixtures/recordings/selective-state-space-run-v0.json',
    caption: 'The SS01 scan with its decay, input, and read gates computed from the token, at state width 32. The last frame holds the learned decay per position on one arithmetic and one prose window, the decay per state dimension, the state along one window, and the argmax.' },
...
const LABELS = {
  ...
  'ssm2/config': { items: ['d_model', 'hidden', 'state', 'T', 'epochs', 'train', 'val', 'wide'] },
  'ssm2/accuracy/val': { items: FAMILIES }, 'ssm2/accuracy/train': { items: FAMILIES },
  'ssm2/logits/example-argmax': { items: ['2', '+', '3', '=', '|', '5', '.'] },
  'ssm2/decay/arith': { items: ['1', '7', '+', '2', '5', '=', '|', '4', '2', '.'] },
  'ssm2/state/example': { rows: ['2', '+', '3', '=', '|', '5', '.'] },
...
const CAPTIONS = {
  ...
  'ssm2/config': 'WHAT: the selective run: the DN01 widths, the state width (32 wide, 16 matched), 300 epochs; the last entry is 1 for the wide variant.',
  'ssm2/loss/train': 'WHAT: the masked loss on the training rows; the wide variant reaches complete memorization (training exact match 1.0 on every family).',
  'ssm2/loss/val': 'WHAT: the masked loss on the 30 held-out rows: 5.63 wide, 4.42 at a matched mixer budget against SS01\'s 4.56 and DN01\'s 3.79.',
  'ssm2/accuracy/val': 'WHAT: held-out exact match by family; the wide variant answers every held-out prose prompt.',
  'ssm2/accuracy/train': 'WHAT: training exact match by family.',
  'ssm2/decay/arith': 'WHAT: the learned decay per position of 17+25=|42., mean over state dimensions: how much of the state each token lets through. HOW: sigmoid(x Wa + b), the bias giving the baseline SS01 has as a constant.',
  'ssm2/decay/prose': 'WHAT: the same for the cat sat on the |mat.; comparing the two strips is the point of the lesson.',
  'ssm2/decay/spread': 'WHAT: the decay per state dimension, averaged over one window: which dimensions are fast and which are slow.',
  'ssm2/state/example': 'WHAT: the selective state after each position of 2+3=|5.',
  'ssm2/logits/example-argmax': 'WHAT: the argmax next token at each position of the example window.',
