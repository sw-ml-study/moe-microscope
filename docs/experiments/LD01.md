...
Note (finding F22, 2026-09-14): the two variants train in one process, but
under distinct model names (`emb2`, `att2`, and so on), and Adam's state is
keyed by name, so LD01r's numbers were not contaminated; the rule going
forward is one training run per process.
